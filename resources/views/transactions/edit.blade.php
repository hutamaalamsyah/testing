...
@section('content')
    <div class="loan-form-page">
        <div class="d-flex justify-content-end mb-3">
            <a href="{{ route('transactions.edit', $transaction) }}" class="btn btn-sm" style="background:#5e87cc;color:#fff;border-radius:10px;width:40px;height:40px;display:inline-grid;place-items:center;">
        ...
        <form action="{{ route('transactions.update', $transaction) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="loan-form-card">
                    ...
                    <div class="col-md-6">
                        <section class="group-section">
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="employee_id" name="employee_id" class="form-control input-pill" value="{{ old('employee_id', $transaction->employee_id ?? '') }}" placeholder="Contoh: MBG0112026001">
                            </div>
                            <div class="form-group mb-0">
                                <label for="division">Divisi</label>
                                <input type="text" id="division" name="division" class="form-control input-pill" value="{{ old('division', $transaction->division ?? '') }}">
                            </div>
                        </section>
                    </div>
                        ...
                        <section class="group-section">
                            ...
                            <div class="form-group">
                                ...
                                <select id="asset_id" name="asset_id" class="form-control input-pill" required>
                                    @foreach ($assets as $asset)
                                        <option value="{{ $asset->id }}" @selected(old('asset_id', $transaction->asset_id) == $asset->id)>
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="text" id="asset_code_preview" name="asset_code_snapshot" class="form-control input-pill" value="{{ old('asset_code_snapshot', $transaction->asset_code_snapshot ?? $transaction->asset->code_asset ?? '') }}">
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="category_preview">Kategori</label>
                                    <input type="text" id="category_preview" name="asset_category_snapshot" class="form-control input-pill" value="{{ old('asset_category_snapshot', $transaction->asset_category_snapshot ?? $transaction->asset->category_asset ?? '') }}">
                                </div>
                            </div>
                            <div class="form-group mb-0">
                                <label for="location_preview">Lokasi Asset</label>
                                <input type="text" id="location_preview" name="asset_location_snapshot" class="form-control input-pill" value="{{ old('asset_location_snapshot', $transaction->asset_location_snapshot ?? $transaction->asset->lokasi_asset ?? '') }}">
                            </div>
                        </section>
                    </div>
                </div>
                        ...
                        <section class="group-section">
                            ...
                            <div class="form-row">
                                <div class="form-group col-md-6">
                                    ...
                                    <input type="date" id="borrowed_at" name="borrowed_at" class="form-control input-pill" value="{{ old('borrowed_at', $transaction->borrowed_at->format('Y-m-d')) }}" required>
                                </div>
                                <div class="form-group col-md-6">
                                    <label for="returned_at">Tanggal Kembali</label>
                                    <input type="date" id="returned_at" name="returned_at" class="form-control input-pill" value="{{ old('returned_at', $transaction->returned_at?->format('Y-m-d')) }}">
                                </div>
                            </div>
                        </section>
                    </div>
                    ...
                    <div class="col-md-6">
                        <section class="group-section">
                            ...
                            <div class="form-group mb-0">
                                ...
                                <input type="text" id="condition_note" name="condition_note" class="form-control input-pill" value="{{ old('condition_note', $transaction->condition_note ?? '') }}">
                            </div>
                        </section>
                    </div>
                </div>
                ...
                <div class="extra-panel">
                    <div class="upload-card">
                        ...
                        <div class="upload-placeholder" id="upload-area" onclick="document.getElementById('photo-input').click()" style="cursor:pointer;position:relative;overflow:hidden;">
                            @if($transaction->photo_path)
                                <img id="photo-preview" src="{{ asset('storage/' . $transaction->photo_path) }}" alt="Foto Aset" style="max-height:120px;max-width:100%;border-radius:12px;">
                    ...
                    <div class="check-card">
                        <div class="custom-control custom-checkbox">
                            <input class="custom-control-input" type="checkbox" id="inspection_confirm" name="inspection_confirmed" value="1" @checked(old('inspection_confirmed', $transaction->inspection_confirmed ?? false))>
                ...
                <input type="hidden" name="cost" value="{{ old('cost', $transaction->cost ?? 0) }}">

                <div class="form-footer">
                    <a href="{{ route('transactions.index') }}" class="btn btn-light btn-cancel">Batal</a>
                    <button type="submit" class="btn btn-save">
                        <i class="fas fa-lock mr-1"></i> Update
                    </button>
                </div>
            </div>
        </form>
